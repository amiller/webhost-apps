...
| login-with-everything | static | source-deploy: `POST /_api/projects` `{"source":"https://github.com/amiller/webhost-apps.git","ref":"login-with-everything-v2"}` | [✓ live](https://78ffc78c25e0c8a9e64bb3a969ba6f226abae62d-8080.dstack-pha-prod7.phala.network/login-with-everything/) | OAuth3 login demo. Pinned public source: tag `login-with-everything-v2` presents this app dir as the repo root, so the daemon's `git clone --branch` gets it (oauth3-apps#14). Docs/PRD stay in the private `amiller/oauth3-apps`. |
