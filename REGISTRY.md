...
| otterscope | deno | prod | tarball → `POST /_api/projects` | [✓ live](https://pod.dstack.soc1024.com/otterscope/) | https://pod.dstack.soc1024.com/otterscope/ | `title "Otter via OAuth3"` | Otter.ai transcript viewer via oauth3 `otter` plugin. **Extension-dependent** (`window.oauth3`) — needs the SDK extension-optional work to run on mobile/same-pod. |
...
| reddit-karma | static | staging | tarball → `POST /_api/projects` (`scripts/deploy-static.sh`) | [✓ live](https://78ffc78c25e0c8a9e64bb3a969ba6f226abae62d-8080.dstack-pha-prod7.phala.network/reddit-karma/) | https://78ffc78c25e0c8a9e64bb3a969ba6f226abae62d-8080.dstack-pha-prod7.phala.network/reddit-karma/ | `title "Reddit Karma"` | **sample OAuth3 app** for the app+plugin story. Connect → reads the Reddit account's karma through a scoped `reddit:read` token. Static `index.html`, same shape as `timeline-peek`. This is the app that motivated the post-merge deploy lane (it sat at 404 after merge until hand-deployed). |
